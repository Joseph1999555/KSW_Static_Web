fetch("header/header.html")
<<<<<<< HEAD
    .then(res => res.text())
    .then(html => {
        document.getElementById("header").innerHTML = html;
        initHeader();
    });

function initHeader() {
    const headerEl = document.querySelector('.site-header');
    const titleEl = document.getElementById('page-title');

    function setHeaderBg(bg) {
        // reset bg classes that start with 'bg-'
        headerEl.className = headerEl.className.split(' ').filter(c => !c.startsWith('bg-')).join(' ');
        if (!bg) return;
        // If bg looks like a hex color, set inline style; otherwise use class 'bg-<name>'
        if (bg.startsWith('#') || bg.startsWith('rgb')) {
            headerEl.style.background = bg;
        } else {
            headerEl.classList.add('bg-' + bg);
            headerEl.style.background = '';
        }
    }

    function updateFromPath() {
        const path = location.hash.replace('#', '') || '/';
        // find link by data-path
        const link = document.querySelector(`.main-nav a[data-path="${path}"]`);
        if (link) {
            const title = link.dataset.title || '';
            const bg = link.dataset.bg || '';
            titleEl.textContent = title;
            setHeaderBg(bg);
        } else {
            // fallback: try to match by href containing the path
            const fallback = Array.from(document.querySelectorAll('.main-nav a')).find(a => a.getAttribute('href')?.includes(path));
            if (fallback) {
                titleEl.textContent = fallback.dataset.title || '';
                setHeaderBg(fallback.dataset.bg || '');
            }
        }
    }

    // initial sync
    updateFromPath();

    // handle SPA navigation clicks inside header
    document.addEventListener('click', e => {
        const link = e.target.closest('[data-link]');
        if (!link) return;

        e.preventDefault();
        const href = link.getAttribute('href');
        // Call the global navigate function (defined in index.js) with href
        navigate(href);

        // update header based on the clicked link's data attributes
        const title = link.dataset.title;
        const bg = link.dataset.bg;
        if (title) titleEl.textContent = title;
        setHeaderBg(bg);
    });

    // handle browser back/forward
    window.addEventListener('popstate', updateFromPath);
=======
  .then(res => res.text())
  .then(html => {
    document.getElementById("header").innerHTML = html;
    initHeader();
  });

function initHeader() {
  const headerEl = document.querySelector('.site-header');
  const titleEl = document.getElementById('page-title');

  function setHeaderBg(bg) {
    // reset bg classes that start with 'bg-'
    headerEl.className = headerEl.className.split(' ').filter(c => !c.startsWith('bg-')).join(' ');
    if (!bg) return;
    // If bg looks like a hex color, set inline style; otherwise use class 'bg-<name>'
    if (bg.startsWith('#') || bg.startsWith('rgb')) {
      headerEl.style.background = bg;
    } else {
      headerEl.classList.add('bg-' + bg);
      headerEl.style.background = '';
    }
  }

  function updateFromPath() {
    const path = location.hash.replace('#', '') || '/';
    // find link by data-path
    const link = document.querySelector(`.main-nav a[data-path="${path}"]`);
    if (link) {
      const title = link.dataset.title || '';
      const bg = link.dataset.bg || '';
      titleEl.textContent = title;
      setHeaderBg(bg);
    } else {
      // fallback: try to match by href containing the path
      const fallback = Array.from(document.querySelectorAll('.main-nav a')).find(a => a.getAttribute('href')?.includes(path));
      if (fallback) {
        titleEl.textContent = fallback.dataset.title || '';
        setHeaderBg(fallback.dataset.bg || '');
      }
    }
  }

  // initial sync
  updateFromPath();

  // handle SPA navigation clicks inside header
  document.addEventListener('click', e => {
    const link = e.target.closest('[data-link]');
    if (!link) return;

    e.preventDefault();
    const href = link.getAttribute('href');
    // Call the global navigate function (defined in index.js) with href
    navigate(href);

    // update header based on the clicked link's data attributes
    const title = link.dataset.title;
    const bg = link.dataset.bg;
    if (title) titleEl.textContent = title;
    setHeaderBg(bg);
  });

  // handle browser back/forward
  window.addEventListener('popstate', updateFromPath);
>>>>>>> 7210a97adae484a8aff3e52398ae090a645a7eac
}

